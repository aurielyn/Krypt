...
object HotbarMapColorParser {
    ...
    fun getTile(arrayX: Int, arrayY: Int): Tile {
        ...
        cachedTiles[index]?.let { return it }
        ...
        val newTile = scanTile(arrayX, arrayY, xPos, zPos)
        cachedTiles[index] = newTile
    ...
    fun getConnected(arrayX: Int, arrayY: Int): List<Room> {
        ...
        while (queue.isNotEmpty()) {
            ...
            queue.addAll(Direction.Type.HORIZONTAL.mapNotNull { direction ->
                getTile(current.x + direction.offsetX, current.z + direction.offsetZ) as? Room
            })
        }
