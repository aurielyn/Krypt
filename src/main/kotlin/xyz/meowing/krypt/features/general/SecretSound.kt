...
object SecretSound : Feature(
    ...
    override fun initialize() {
        register<PacketEvent.Sent> { event ->
            ...
            if (
                blockState.block == Blocks.CHEST ||
                blockState.block == Blocks.TRAPPED_CHEST ||
                blockState.block == Blocks.LEVER ||
                checkSkull(world, pos)
                ) {
                playSound()
            }
        }
        ...
        register<PacketEvent.Received> { event ->
            ...
            if (
                secretTypes.binarySearch(sanitizedName) >= 0 &&
                player.distanceTo(entity) <= secretDistance
                ) {
                playSound()
            }
        }
